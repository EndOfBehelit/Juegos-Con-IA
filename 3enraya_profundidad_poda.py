...
#Minimax
def minimax(tablero, jugador, profundidad=0, alpha=float('-inf'), beta=float('inf')):
    ...
    if jugador:
        mejor_puntuacion = (float('-inf'), 10)
        for i in pos_vacias:
            tablero[i[0]][i[1]] = 'X'
            resultado = minimax(tablero, False, profundidad + 1, alpha, beta)
            tablero[i[0]][i[1]] = ' '

            if resultado[0] > mejor_puntuacion[0] or (resultado[0] == mejor_puntuacion[0] and resultado[1] < mejor_puntuacion[1]):
                mejor_puntuacion = resultado

            alpha = max(alpha, mejor_puntuacion[0])
            if beta <= alpha:
                break  
        ...
        for i in pos_vacias:
            tablero[i[0]][i[1]] = 'O'
            resultado = minimax(tablero, True, profundidad + 1, alpha, beta)
            tablero[i[0]][i[1]] = ' '

            if resultado[0] < mejor_puntuacion[0] or (resultado[0] == mejor_puntuacion[0] and resultado[1] < mejor_puntuacion[1]):
                mejor_puntuacion = resultado

            beta = min(beta, mejor_puntuacion[0])
            if beta <= alpha:
                break  
